 ...
 */
#define MEMP_MEM_MALLOC             1//0 /* 使用动态内存池 pool 的方式 给 TCP 以及 UDP 控制块，内核在初始化的时候为每个数据结构初始化好了一定的pool */ 
...
/* MEMP_NUM_PBUF: the number of memp struct pbufs. If the application
   ...
   should be set high. */
#define MEMP_NUM_PBUF           20 /* 应用程序 发送大量数据 是存在ROM 的，比如 网页，这个值应该设置大一点 */
...
#define MEMP_NUM_UDP_PCB        6 /* 上层API 可以使用 UDP 的个数，UDP 连接较多时 应该增大该值 */

/* MEMP_NUM_TCP_PCB: the number of simulatenously active TCP
   connections. */
#define MEMP_NUM_TCP_PCB        10 /* 上层API 可以使用 TCP 的个数，UDP 连接较多时 应该增大该值 */

/* MEMP_NUM_TCP_PCB_LISTEN: the number of listening TCP
   connections. */
#define MEMP_NUM_TCP_PCB_LISTEN 6 /* 上层API 可以使用 TCP 监听的个数，TCP 服务器较多时 应该增大该值 */
...
#define PBUF_POOL_SIZE          20 /* pool 类型的 PUBF 的个数，建议接收驱动中使用该类型的PBUF数据包，并加大该值  */
...
#define  TCP_KEEPINTVL_DEFAULT    100UL//1000UL		 // 每1秒发送一次保活探测（该值默认为75S）
